...
Version at time of writing: **0.5.4**. Read this before changing anything;
...
### Licensing
- **Nothing enforces the licence.** `core/licence.py` reports; it does not
  restrict. A test asserts an expired trial still builds a canvas, so
  enforcement cannot creep in unnoticed.
- **Do not switch enforcement on without signature verification.** The file is
  plain JSON; `verify()` returns False by design. Enforcing an unsigned
  licence is decoration -- a user edits the date in ten seconds.
- **A missing expiry falls back to the trial**, never to "unlimited".
