...
class DatasetTreeModel(QAbstractItemModel):
    ...
    def canFetchMore(self, parent: QModelIndex) -> bool:
        """Always False: population is driven explicitly by DatasetBrowser via
        DatasetPopulator, NOT by Qt's automatic fetch-on-expand.

        Qt calls canFetchMore/fetchMore synchronously on the GUI thread the
        moment a node is expanded. If this returned True, that synchronous
        fetchMore would (a) run the directory scan on the GUI thread -- the
        very network-blocking call the async populator exists to avoid -- and
        (b) mark the node fetched before the browser's own expand handler
        runs, so the handler would skip scheduling the background metadata
        probes entirely. That second effect was the bug behind "columns never
        fill in and I can't drag": rows appeared (via the synchronous fetch)
        but were never probed, so their dimensionality stayed unknown and they
        were not draggable. Returning False keeps Qt's hands off; the browser
        expands -> populator scans on a worker -> applyChildren -> probes.
        """
        return False
    ...
    def fetchMore(self, parent: QModelIndex) -> None:
        ...
        No longer driven by Qt (canFetchMore is always False). Retained as the
        synchronous scan used directly by tests and by DatasetPopulator's
        worker path via _scan_children/applyChildren. Production expansion goes
        through DatasetPopulator, which runs the scan off the GUI thread.
        """
        if not parent.isValid():
            return
        node: Node = parent.internalPointer()
