...
def test_canFetchMore_is_always_false_population_is_browser_driven(two_samples):
    """canFetchMore is deliberately always False. Qt calls fetchMore
    synchronously on the GUI thread when it returns True, which would run the
    directory scan on the GUI thread (blocking on network paths) and mark the
    node fetched before the browser could schedule background metadata probes.
    Population is instead driven explicitly by DatasetBrowser via
    DatasetPopulator (async scan + probe scheduling)."""
    model = model_for(two_samples)
    root_idx = model.index(0, 0)
    node = root_idx.internalPointer()

    assert not node.is_fetched
    assert not model.canFetchMore(root_idx)   # even though not fetched

    expand(model, root_idx)
    assert node.is_fetched
    assert not model.canFetchMore(root_idx)   # and still false after
