...
def test_recent_ranges_are_shown_low_to_high(qtbot):
    """Reported as "always reversed": the stored pair is (high, low) because
    that is the order the descending ppm axis needs, but displaying it that
    way made every entry read backwards. Shown low-to-high, as a range is
    read aloud."""
    bar = AdjustmentBar()
    bar.set_enabled_for_figure(True)
