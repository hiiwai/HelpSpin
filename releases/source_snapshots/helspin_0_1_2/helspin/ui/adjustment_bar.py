...
class AdjustmentBar(QWidget):
    ...
    def _remember(self, left: float, right: float) -> None:
        ...
        # Shown LOW to HIGH, which is how a range is read aloud ("1 to 12
        # ppm"). The stored pair stays (high, low) because that is the order
        # the plot needs -- ppm axes run high on the left. Displaying the
        # stored order made every entry look reversed.
