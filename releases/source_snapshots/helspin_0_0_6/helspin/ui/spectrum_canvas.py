...
class SpectrumCanvas(QWidget):
    ...
    def _redraw(self) -> None:
        ...
        # Y limits are pinned rather than left to autoscale. Autoscaling makes
        # a per-trace vertical scale invisible: multiply the data and the axis
        # simply grows to match, so nothing appears to change.
        if self._y_limits is None:
            self._y_limits = self._natural_y_limits(drawn)
        ...
        if self._show_grid:
            ...
            self._axes.grid(
                True, which="major", linewidth=0.4, alpha=0.25, linestyle="-"
            )
        else:
    ...
    def _natural_y_limits(self, drawn):
        """Y range that comfortably fits everything currently drawn."""
        if not drawn:
            return None
        lows, highs = [], []
        for _trace, y in drawn:
            if y.size == 0:
                continue
            lows.append(float(np.nanmin(y)))
            highs.append(float(np.nanmax(y)))
        if not lows:
            return None
    ...
    def _draw_trace_labels(self, drawn, left: float, right: float) -> None:
        """Name each spectrum at its own top-left, in the trace's colour."""
        if not drawn:
            return
        # Just inside the left (high-ppm) edge.
        span = left - right
        x = left - span * 0.02 if span != 0 else left
        for trace, y in drawn:
            if y.size == 0:
                continue
            in_view = (trace.ppm <= left) & (trace.ppm >= right)
            segment = y[in_view] if np.any(in_view) else y
            try:
                top = float(np.nanmax(segment))
            except ValueError:
                continue
