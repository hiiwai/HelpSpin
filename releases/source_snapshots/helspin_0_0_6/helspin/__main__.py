...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._horizontal_split.setSizes([260, 640, 200])
        ...
        self._spectrum_list.lineStyleChanged.connect(
            self._canvas.set_trace_line_style
        )
    ...
    def _build_toolbar(self) -> None:
        ...
        # ONE toggle that flips between overlay and stacked, rather than two
        # buttons where both could look unset. Its label always names the
        # CURRENT mode, so there is no ambiguity about what you are looking at.
        self._arrangement_action = toolbar.addAction(
            "Overlay", self._toggle_arrangement
        )
        self._arrangement_action.setToolTip("Click to switch to Stacked")
    ...
    def _toggle_arrangement(self) -> None:
        """Flip between overlay and stacked."""
        if self._canvas.arrangement() == SpectrumCanvas.ARRANGEMENT_OVERLAY:
        ...
        self._sync_arrangement_label()

    def _sync_arrangement_label(self) -> None:
        """The button shows the CURRENT mode and the tooltip says what a click
        will do -- a checkable button labelled with the other mode is the
        classic source of 'which one am I in?' confusion."""
        if self._canvas.arrangement() == SpectrumCanvas.ARRANGEMENT_OVERLAY:
            self._arrangement_action.setText("Overlay")
            self._arrangement_action.setToolTip("Click to switch to Stacked")
        else:
            self._arrangement_action.setText("Stacked")
            self._arrangement_action.setToolTip("Click to switch to Overlay")
    ...
    def _preferences(self) -> None:
        """Line width and colour cycle -- the settings that actually change
        the figure today. Cancel changes nothing; the dialog itself has no
        side effects, values are only applied here on accept."""
        dialog = PreferencesDialog(
            line_width=self._canvas.default_line_width(),
            colors=self._canvas._active_palette(),
            parent=self,
        )
        if dialog.exec() != PreferencesDialog.Accepted:
            return
        self._canvas.set_default_line_width(dialog.line_width())
        self._canvas.set_palette(dialog.colors())
        self._sync_spectrum_list()
