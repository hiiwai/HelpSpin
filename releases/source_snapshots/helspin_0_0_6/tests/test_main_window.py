...
def test_toolbar_has_the_expected_actions(qtbot):
    ...
    assert any(t in ("Overlay", "Stacked") for t in labels)
...
def test_arrangement_toggle_flips_between_overlay_and_stacked(qtbot):
    """One button, not two: its label always names the CURRENT mode and the
    tooltip says what clicking will do, so there is no 'which am I in?'
    ambiguity."""
    from helspin.ui.spectrum_canvas import SpectrumCanvas
    ...
    assert window._arrangement_action.text() == "Overlay"
    assert "Stacked" in window._arrangement_action.toolTip()

    window._toggle_arrangement()
    assert window._canvas.arrangement() == SpectrumCanvas.ARRANGEMENT_STACKED
    assert window._arrangement_action.text() == "Stacked"
    assert "Overlay" in window._arrangement_action.toolTip()
