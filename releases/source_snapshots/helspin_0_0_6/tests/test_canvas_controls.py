...
def test_reset_y_limits_recomputes_from_current_data(qtbot):
    canvas = loaded_canvas(qtbot, 1)
    canvas.set_y_scale(0, 10.0)
    canvas.reset_y_limits()
    low, high = canvas._axes.get_ylim()
    assert high > 900     # now fits the scaled trace
...
def test_drag_does_nothing_in_overlay_mode(qtbot):
    """Which spectrum a drag refers to is ambiguous in overlay, so it is
    deliberately inert there rather than moving an arbitrary trace."""
    canvas = loaded_canvas(qtbot, 2)
    canvas.select_trace(1)
    canvas._on_mouse_press(FakeMouseEvent(canvas._axes, ydata=10.0))
