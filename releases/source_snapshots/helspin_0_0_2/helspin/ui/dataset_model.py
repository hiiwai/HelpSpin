...
class DatasetTreeModel(QAbstractItemModel):
    ...
    def probe_node(self, node: Node) -> None:
        """Read one expno's acqus and fill its metadata columns.

        Runs the actual (potentially slow, network-bound) probe for a single
        row. Called from a background worker per visible row, NOT during
        _scan_sample -- that is what keeps expansion instant. Emits
        dataChanged so the row's columns refresh in place when done.
        """
        if node.kind is not NodeKind.EXPNO or not node.probe_pending:
            return
        try:
            node.info = self._reader.probe(node.path, data_root=_owning_root(node))
...
class _ProbeSignals(QObject):
    done = Signal(object)   # node


class _ProbeTask(QRunnable):
    """Reads one expno's acqus off the GUI thread.

    The heavy work (probe_node -> reader.probe -> acqus read) runs here on
    the worker; the dataChanged emit it does at the end is queued back to the
    GUI thread by Qt because the signal crosses threads. This is what lets 30
    expnos' rows appear instantly and then fill their columns independently,
    instead of blocking on 30 sequential acqus reads before showing anything.
    """
    ...
    def run(self) -> None:
        self._model.probe_node(self._node)
        self.signals.done.emit(self._node)
