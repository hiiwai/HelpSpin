...
def status_line() -> str:
    """The licence in force, as one line. Never raises: a licence system that
    can stop the application opening a dialog is worse than none."""
    try:
        from ..core.licence import current_licence

        licence = current_licence()
        text = licence.describe()
        if licence.problem:
            text += f"  [{licence.problem}]"
        return text
    except Exception:      # noqa: BLE001 - reporting must not break the dialog
...
class LicenceDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Status line first: which licence is in force and when it lapses.
        # Nothing enforces it -- this reports, it does not restrict.
