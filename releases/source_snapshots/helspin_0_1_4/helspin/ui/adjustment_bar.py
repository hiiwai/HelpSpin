...
class AdjustmentBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._f1_label = QLabel("F1  left")
        self._f1_right_label = QLabel("right")
    ...
    def set_f1_range(self, left: float, right: float) -> None:
        high, low = (left, right) if left > right else (right, left)
        for spin, value in ((self._f1_left, high), (self._f1_right, low)):
            spin.blockSignals(True)
            spin.setValue(value)
            spin.blockSignals(False)

    def f1_range(self) -> tuple:
        return (self._f1_left.value(), self._f1_right.value())
    ...
    def _remember(self, left: float, right: float) -> None:
        ...
        # Displayed HIGH to LOW, left to right -- the same order as the axis
        # and the same order as the two boxes. Showing a different order from
        # the plot is what made these look wrong.
        self._recent_combo.addItems(
            [f"{max(l, r):.2f} \u2192 {min(l, r):.2f} ppm" for l, r in self._recent]
        )
