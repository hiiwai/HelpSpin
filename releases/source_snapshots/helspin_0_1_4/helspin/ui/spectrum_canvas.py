...
class SpectrumCanvas(QWidget):
    ...
    def move_to_bottom(self, index: int) -> None:
        ...
        if trace.is_2d or trace.intensity.size == 0:
            return   # a contour panel has no baseline to move
        frame = self._y_limits or self._frame_y_limits(self._visible_1d(), 0.0)
        if frame is None:
            return
    ...
    def _combine(self, index_a: int, index_b: int, op: str,
        ...
        ya, yb = resample(a), resample(b)
        combined = ya + yb if op == "+" else ya - yb
        symbol = "+" if op == "+" else "\u2212"
