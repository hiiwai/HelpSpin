...
class SpectrumCanvas(QWidget):
    ...
    def save_image(self, path, transparent: bool = False, dpi: int = 300) -> None:
        ...
        if suffix == "pptx":
            ...
            return
        if suffix == "emf":
            self._save_emf(path, overrides=overrides)
            return
    ...
    def _save_emf(self, path: str, overrides: dict) -> None:
        """EMF via Inkscape.

        matplotlib cannot write EMF -- it is genuinely not among its output
        formats -- so the figure is written as SVG and converted. Inkscape is
        the only widely available converter that preserves vector content and
        text. If it is not installed the error says exactly that, rather than
        ...
        if not inkscape:
            raise RuntimeError(
                "EMF export requires Inkscape (matplotlib cannot write EMF). "
                "Install Inkscape, or use SVG/PDF/EPS, which are vector too."
            )
        with _tempfile.TemporaryDirectory() as tmp:
            svg = f"{tmp}/figure.svg"
            with mpl.rc_context(overrides):
                self._figure.savefig(
                    svg, format="svg", facecolor="white", edgecolor="none"
                )
            result = subprocess.run(
                [inkscape, svg, "--export-type=emf", f"--export-filename={path}"],
                capture_output=True, text=True, timeout=120,
            )
            if result.returncode != 0:
                raise RuntimeError(
                    f"Inkscape could not convert to EMF: {result.stderr.strip()}"
                )
