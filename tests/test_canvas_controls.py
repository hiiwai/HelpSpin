...
def test_grid_covers_both_axes_in_overlay(qtbot):
    """Was x-only, on the reasoning that a horizontal grid cuts across a
    stacked plot. True when stacked -- and wrong in overlay, where the y axis
    is a single shared intensity scale and a reader wants to measure against
    it. Now: both axes in overlay, x only when stacked."""
    canvas = loaded_canvas(qtbot, 1)
    canvas.set_grid_visible(True)
...
def test_export_filename_gets_an_extension_on_windows_style_paths(qtbot):
    ...
    Example Corp", a dotted user name -- read as an extension already present
    ...
    assert not PureWindowsPath(r"C:\Users\h.ide\OneDrive - Company\figure").suffix
...
# --- palettes and preference persistence ------------------------------------


def test_a_palette_fills_all_eight_colour_slots(qtbot):
    """Setting eight colours by hand is tedious and tends to produce a set
    that is not actually distinguishable; these are published schemes designed
    for exactly that."""
    ...
    dialog._palette_box.setCurrentText("Tol muted")
    dialog._apply_palette()

    expected = [c.upper() for c in palette_colours("Tol muted")]
    assert [s["color"].upper() for s in dialog.styles()] == expected
    assert dialog.palette() == "Tol muted"


def test_the_greyscale_palette_also_sets_line_styles(qtbot):
    """In a figure printed in black and white the dash pattern is the only
    thing telling two spectra apart, so that palette is unusable without it."""
    from helspin.ui.preferences_dialog import PreferencesDialog
    ...
    applied = [s["style"] for s in dialog.styles()]
    assert len(set(applied)) > 1, "the dashes must differ, or nothing is legible"


def test_other_palettes_leave_hand_set_line_styles_alone(qtbot):
    from helspin.ui.preferences_dialog import PreferencesDialog

    dialog = PreferencesDialog()
    qtbot.addWidget(dialog)
    before = [s["style"] for s in dialog.styles()]
    dialog._palette_box.setCurrentText("High contrast")
    dialog._apply_palette()
    assert [s["style"] for s in dialog.styles()] == before


def test_opening_the_palette_list_does_not_change_anything(qtbot):
    """Applying is an explicit button press, so reading the names in the
    dropdown cannot overwrite colours already chosen by hand."""
    from helspin.ui.preferences_dialog import PreferencesDialog

    dialog = PreferencesDialog()
    qtbot.addWidget(dialog)
    before = [s["color"] for s in dialog.styles()]
    dialog._palette_box.setCurrentText("Matplotlib tab10")
...
def test_display_preferences_survive_a_restart(tmp_path, monkeypatch):
    """Only the slot styles were persisted; grid spacing, opacity, cursor
    decimals and the rest were applied to the canvas and never written, so the
    dialog appeared to work and silently reverted on the next launch."""
    from PySide6.QtCore import QSettings

    from helspin.core import settings as settings_module

    monkeypatch.setattr(
        settings_module, "_settings",
        lambda: QSettings(str(tmp_path / "prefs.ini"), QSettings.Format.IniFormat),
    )

    settings_module.save_display_prefs({
        "grid_spacing_ppm": 0.5,
        "grid_spacing_y": 2.5,
        "x_decimals": 3,
        "label_scale": 1.4,
        "opacity": 0.9,
    ...
    restored = settings_module.load_display_prefs()
    assert restored["grid_spacing_ppm"] == pytest.approx(0.5)
    assert restored["grid_spacing_y"] == pytest.approx(2.5)
    assert restored["opacity"] == pytest.approx(0.9)
    assert restored["cursor_decimals"] == 3
    assert restored["palette"] == "Tol muted"


def test_a_corrupt_preferences_entry_falls_back_to_defaults(tmp_path, monkeypatch):
    """A settings file is user-editable and survives upgrades, so what comes
    ...
    monkeypatch.setattr(settings_module, "_settings", lambda: store)

    store.setValue(settings_module._DISPLAY_KEY, "{ not json")
    assert settings_module.load_display_prefs() == {}

    store.setValue(
        settings_module._DISPLAY_KEY,
        '{"opacity": "not a number", "cursor_decimals": 3}',
    )
