...
def make_expno(sample: Path, expno: int, pulprog="zg30") -> Path:
    ...
    # Real Bruker expnos carry processed data; the browser filters out ones
    # that do not (they cannot be displayed), so fixtures must include it.
    procno = d / "pdata" / "1"
...
def test_filter_narrows_visible_samples(qtbot, tmp_path):
    root_dir = tmp_path / "600"
    make_expno(root_dir / "SampleB", 11)
    ...
    browser._filter_edit.setText("SampleB")
    qtbot.wait(50)
...
def test_expanding_via_the_tree_probes_and_enables_drag(qtbot, data_root):
    """Regression for the real reported bug: expanding through the tree view
    (as a user does), NOT via _request_populate directly, must still probe the
    expno rows so their metadata fills in and they become draggable.

    Qt calls fetchMore synchronously on expand; an earlier version let that
    populate the rows and mark the node fetched before the browser scheduled
    probes, so rows appeared but were never probed -- columns stayed on the
    '…' placeholder and drag produced an empty payload. This test expands the
    way the GUI does and asserts a real drag payload results. It is the test
    the _request_populate-based tests could not be, because they bypassed the
    exact path that was broken."""
    import json

    from helspin.ui.dataset_model import COL_PULPROG, MIME_DATASET
    ...
    # Expand through the tree, exactly as a user click does.
    browser.tree.expand(browser._proxy.mapFromSource(root_index))
    sample_index = browser.model.index(0, 0, root_index)
    browser.tree.expand(browser._proxy.mapFromSource(sample_index))
    qtbot.waitUntil(lambda: browser.model.rowCount(sample_index) == 2, timeout=2000)

    # Metadata must actually fill in (probe ran).
    qtbot.waitUntil(
        lambda: browser.model.data(browser.model.index(0, COL_PULPROG, sample_index)) != "\u2026",
        timeout=2000,
    )

    # And a drag built from the probed rows carries a real payload.
    proxy_indices = [
        browser._proxy.mapFromSource(browser.model.index(r, 0, sample_index))
        for r in range(browser.model.rowCount(sample_index))
    ]
    drag = browser.tree._build_drag(proxy_indices)
    assert drag is not None
    payload = json.loads(bytes(drag.mimeData().data(MIME_DATASET)).decode("utf-8"))
    assert len(payload) == 2   # both expnos, with known dimensionality
...
def test_refresh_all_preserves_expansion_and_merges_new_expnos(qtbot, tmp_path):
    """The reported bug, reproduced end-to-end: with a sample expanded, adding
    a new expno on disk and hitting Refresh All must (a) keep the sample
    expanded and (b) show the new expno -- previously refresh collapsed every
    open folder and the new file only appeared after an app restart."""
    root_dir = tmp_path / "600"
    make_expno(root_dir / "sample_a", 11)
    make_expno(root_dir / "sample_a", 12)
    make_expno(root_dir / "sample_b", 21)
    browser = DatasetBrowser([DataRoot(name="600", path=root_dir)])
    qtbot.addWidget(browser)
    ...
    # Expand the data root, then sample_a.
    browser.tree.expand(browser._proxy.mapFromSource(root_index))
    sample_a_index = next(
        browser.model.index(r, 0, root_index)
        for r in range(browser.model.rowCount(root_index))
        if browser.model.index(r, 0, root_index).internalPointer().display_name == "sample_a"
    )
    browser.tree.expand(browser._proxy.mapFromSource(sample_a_index))
    qtbot.waitUntil(lambda: browser.model.rowCount(sample_a_index) == 2, timeout=2000)
    assert browser.tree.isExpanded(browser._proxy.mapFromSource(sample_a_index))

    # New expno in the expanded sample; new sample at the root.
    make_expno(root_dir / "sample_a", 13)
    make_expno(root_dir / "sample_c", 31)
    browser.refresh_all()

    # sample_a stays expanded and gains the third expno.
    qtbot.waitUntil(lambda: browser.model.rowCount(sample_a_index) == 3, timeout=2000)
    assert browser.tree.isExpanded(browser._proxy.mapFromSource(sample_a_index))
    # ...and the new sample appears at the root.
    qtbot.waitUntil(lambda: browser.model.rowCount(root_index) == 3, timeout=2000)


def test_refresh_preserves_existing_expno_node_identity(qtbot, tmp_path):
    """Merge keeps existing expno node OBJECTS rather than recreating them, so
    whatever state they hold (probed metadata, etc.) survives a refresh
    instead of being thrown away and reloaded. Checked via object identity,
    which is the concrete merge guarantee and does not depend on async probe
    timing."""
    root_dir = tmp_path / "600"
    make_expno(root_dir / "sample_a", 11)
    make_expno(root_dir / "sample_a", 12)
    ...
    # Capture the existing expno node objects.
    before = {
        browser.model.index(r, 0, sample_index).internalPointer().display_name:
            browser.model.index(r, 0, sample_index).internalPointer()
        for r in range(browser.model.rowCount(sample_index))
    }

    # Add a third expno and refresh.
    make_expno(root_dir / "sample_a", 13)
    browser.refresh_all()
    qtbot.waitUntil(lambda: browser.model.rowCount(sample_index) == 3, timeout=2000)

    # The two original expno nodes are the SAME objects, not recreated.
    after = {
        browser.model.index(r, 0, sample_index).internalPointer().display_name:
            browser.model.index(r, 0, sample_index).internalPointer()
        for r in range(browser.model.rowCount(sample_index))
    }
    assert after["11"] is before["11"]
    assert after["12"] is before["12"]
    assert "13" in after   # the new one was merged in
...
def test_context_menu_offers_removal_only_for_a_data_root(qtbot, data_root):
    browser = DatasetBrowser([DataRoot(name="600 MHz", path=data_root)])
    qtbot.addWidget(browser)
    root_index = browser.model.index(0, 0)
    qtbot.waitUntil(lambda: browser.model.rowCount(root_index) == 1, timeout=2000)
    ...
    root_labels = [
        a.text() for a in browser._build_context_menu(root_index).actions()
        if a.text()
    ]
    assert any("Remove data root" in t for t in root_labels)

    sample_index = browser.model.index(0, 0, root_index)
    sample_labels = [
        a.text() for a in browser._build_context_menu(sample_index).actions()
        if a.text()
    ]
    assert not any("Remove data root" in t for t in sample_labels)


def test_removing_a_data_root_updates_the_model_and_signals(qtbot, data_root):
    browser = DatasetBrowser([DataRoot(name="600 MHz", path=data_root)])
    qtbot.addWidget(browser)
    node = browser.model.index(0, 0).internalPointer()
    with qtbot.waitSignal(browser.dataRootRemoved, timeout=1000):
        browser.remove_data_root_node(node)
    assert browser.model.rowCount() == 0
    assert browser.data_roots() == []
